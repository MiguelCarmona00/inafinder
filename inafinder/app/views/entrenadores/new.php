...
<head>
    ...
    <title>Añadir Entrenadores</title>
...
<body class="bg-primary font-sans text-white min-h-screen pt-16">
    ...
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-4xl mx-auto">
            <!-- Botón para ver lista de entrenadores -->
            
            <div class="flex justify-center mb-6">
                <a href="/entrenadoresList" class="bg-info px-6 py-3 bg-warning text-white rounded-lg hover:bg-yellow-600 transition-colors font-medium focus:ring-2 focus:ring-warning">
                    📋 Ver Lista de Entrenadores
                </a>
            </div>

            <h1 class="text-3xl font-bold mb-6 text-center">Añadir Entrenadores</h1>

            <form action="/entrenadores" method="POST" enctype="multipart/form-data" id="mainForm">
                ...
                <div id="formsContainer">
                    ...
                    <div class="entrenador-form bg-darker rounded-lg p-6 mb-6 shadow-[0_2px_8px_rgba(0,0,0,0.3)] relative" data-form-index="0">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-xl font-bold">Entrenador #1</h3>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="file" name="entrenadores[0][imagen]" accept="image/*" required 
                            ...
                            <div>
                                ...
                                <input type="text" name="entrenadores[0][nombre]" required 
                ...
                <div class="flex gap-4 pt-4 justify-center">
                    <button type="submit" 
                        ...
                        🛡️ Añadir Todos los Entrenadores
                    </button>
                ...
                <div class="flex justify-center mt-3">
                    <a href="/entrenadoresList" class="px-8 py-3 bg-gray text-white rounded-lg hover:bg-light-gray transition-colors font-medium text-center focus:ring-2 focus:ring-gray focus:ring-offset-2 focus:ring-offset-darker">
                        Cancelar
                    </a>
                </div>
            </form>
    ...
    $(document).ready(function() {
        ...
        function updateFormIndices() {
            $('.entrenador-form').each(function(index) {
                $(this).attr('data-form-index', index);
                $(this).find('h3').text('Entrenador #' + (index + 1));
                
                // Actualizar nombres de los campos
                $(this).find('input').each(function() {
                    const name = $(this).attr('name');
                    if (name && name.includes('entrenadores[')) {
                        const newName = name.replace(/entrenadores\[\d+\]/, 'entrenadores[' + index + ']');
        ...
        $('#addForm').click(function() {
            if (formCount < maxForms) {
                const newForm = $('.entrenador-form').first().clone();
                
                // Limpiar valores del formulario clonado
                newForm.find('input[type="text"], input[type="file"]').val('');
        ...
        $(document).on('click', '.removeForm', function() {
            if (formCount > 1) {
                $(this).closest('.entrenador-form').fadeOut(300, function() {
                    $(this).remove();
                    formCount--;
                    updateFormIndices();
                    updateCounter();
        ...
        $('#mainForm').submit(function(e) {
            const message = formCount === 1 ? 
                '¿Estás seguro de que quieres añadir este entrenaodor?' : 
                `¿Estás seguro de que quieres añadir estos ${formCount} entrenadores?`;
