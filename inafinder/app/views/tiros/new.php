...
<body class="bg-primary font-sans text-white min-h-screen pt-16">
    ...
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-4xl mx-auto">
            <!-- Botón para ver lista de tiros -->
            <div class="flex justify-center mb-6">
                <a href="/tirosList" class="bg-info px-6 py-3 bg-warning text-white rounded-lg hover:bg-yellow-600 transition-colors font-medium focus:ring-2 focus:ring-warning">
                    📋 Ver Lista de Tiros
                </a>
            </div>
            
            <h1 class="text-3xl font-bold mb-6 text-center">Añadir Tiros</h1>

            <form action="/tiros" method="POST" enctype="multipart/form-data" id="mainForm">
                ...
                <div id="formsContainer">
                    ...
                    <div class="tiro-form bg-darker rounded-lg p-6 mb-6 shadow-[0_2px_8px_rgba(0,0,0,0.3)] relative" data-form-index="0">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-xl font-bold">Tiro #1</h3>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="file" name="tiros[0][imagen]" accept="image/*" required 
                            ...
                            <div>
                                ...
                                <input type="text" name="tiros[0][nombre]" required 
                                       placeholder="Ej: Cañonazo de fragmentos, Tiro supermasivo..."
                            ...
                            <div>
                                ...
                                <select name="tiros[0][elemento]" required 
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    <label class="flex items-center p-3 bg-gray rounded-lg border border-light-gray hover:border-success transition-colors cursor-pointer">
                                        <input type="radio" name="tiros[0][subtipo]" value="largo" required 
                                               class="mr-3 text-success focus:ring-success focus:ring-2">
                                        <span class="text-white">🏹 Largo</span>
                                    </label>
                                    <label class="flex items-center p-3 bg-gray rounded-lg border border-light-gray hover:border-success transition-colors cursor-pointer">
                                        <input type="radio" name="tiros[0][subtipo]" value="bloqueo" required 
                                               class="mr-3 text-success focus:ring-success focus:ring-2">
                                        <span class="text-white">🛡️ Bloqueo</span>
                                    </label>
                                    <label class="flex items-center p-3 bg-gray rounded-lg border border-light-gray hover:border-success transition-colors cursor-pointer">
                                        <input type="radio" name="tiros[0][subtipo]" value="cadena" required 
                                               class="mr-3 text-success focus:ring-success focus:ring-2">
                                        <span class="text-white">⛓️ Cadena</span>
                                    </label>
                                    <label class="flex items-center p-3 bg-gray rounded-lg border border-light-gray hover:border-success transition-colors cursor-pointer">
                                        <input type="radio" name="tiros[0][subtipo]" value="normal" required 
                                               class="mr-3 text-success focus:ring-success focus:ring-2">
                            ...
                            <div>
                                ...
                                <input type="number" name="tiros[0][potencia]" required min="0" max="999"
                                       placeholder="Ej: 95"
                                       class="w-full p-3 bg-gray text-white border border-light-gray rounded-lg focus:border-success focus:ring-1 focus:ring-success transition-colors placeholder-light-gray">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="tiros[0][costo]" required min="0" max="999"
                                       placeholder="Ej: 25"
                                       class="w-full p-3 bg-gray text-white border border-light-gray rounded-lg focus:border-success focus:ring-1 focus:ring-success transition-colors placeholder-light-gray">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="tiros[0][dificultad]" required
                                       placeholder="Ej: 5"
                                       class="w-full p-3 bg-gray text-white border border-light-gray rounded-lg focus:border-success focus:ring-1 focus:ring-success transition-colors placeholder-light-gray">
                                <p class="text-xs text-light-gray mt-1">Nivel de dificultad (sin límites)</p>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="tiros[0][n_jugadores]" required 
                                        class="w-full p-3 bg-gray text-white border border-light-gray rounded-lg focus:border-success focus:ring-1 focus:ring-success transition-colors">
    ...
    $(document).ready(function() {
        ...
        function updateFormIndices() {
            $('.tiro-form').each(function(index) {
                $(this).attr('data-form-index', index);
                $(this).find('h3').text('Tiro #' + (index + 1));
                
                // Actualizar nombres de los campos
                $(this).find('input, select').each(function() {
                    const name = $(this).attr('name');
                    if (name && name.includes('tiros[')) {
                        const newName = name.replace(/tiros\[\d+\]/, 'tiros[' + index + ']');
        ...
        $('#addForm').click(function() {
            if (formCount < maxForms) {
                const newForm = $('.tiro-form').first().clone();
        ...
        $(document).on('click', '.removeForm', function() {
            if (formCount > 1) {
                $(this).closest('.tiro-form').fadeOut(300, function() {
                    $(this).remove();
                    formCount--;
                    updateFormIndices();
                    updateCounter();
        ...
        $('#mainForm').submit(function(e) {
            const message = formCount === 1 ? 
                '¿Estás seguro de que quieres añadir este tiro?' : 
                `¿Estás seguro de que quieres añadir estos ${formCount} tiros?`;
